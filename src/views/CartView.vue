...
<template>
  <main class="py-8">
    ...
    <div class="flex flex-col gap-y-4">
      <div v-for="(item, idx) in cartItems" :key="idx" class="grid grid-cols-12 border shadow-md">
        ...
        <div class="card-column col-span-2">
          <button class="button" @click="cartStore.removeItemFromCart(item.name)">delete</button>
        </div>
      </div>
      <router-link :to="{ path: '/placeorder' }"
        ><button class="button w-full">Check Out</button>
      </router-link>
    </div>
  </main>
</template>
